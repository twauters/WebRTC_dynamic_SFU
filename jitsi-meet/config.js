...
//    configLocation: './config.json', // see ./modules/HttpConfigFetch.js
    hosts: {
        ...
        muc: 'conference.jitsi', // FIXME: use XEP-0030
        //jirecon: 'jirecon.jitsi-meet.example.com',
        //call_control: 'callcontrol.jitsi-meet.example.com',
        focus: 'focus.jitsi', // defaults to 'focus.jitsi-meet.example.com'
    },
//  getroomnode: function (path) { return 'someprefixpossiblybasedonpath'; },
//  useStunTurn: true, // use XEP-0215 to fetch STUN and TURN server
//  useIPv6: true, // ipv6 support. use at your own risk
    useNicks: false,
    bosh: '//jitsi/http-bind', // FIXME: use xep-0156 for that
    ...
    channelLastN: (decodeURIComponent((new RegExp('[?|&]sendonly=' + '([^&;]+?)(&|#|;|$)').exec(location.search)||[,""])[1].replace(/\+/g, '%20')) === 'true') ? 0 : 1, // The default value of the channel attribute last-n.
    enableRecording: false,
    enableWelcomePage: false,
    enableClosePage: false, // enabling the close page will ignore the welcome
                              // page redirection when call is hangup
    disableSimulcast: (decodeURIComponent((new RegExp('[?|&]simulcast=' + '([^&;]+?)(&|#|;|$)').exec(location.search)||[,""])[1].replace(/\+/g, '%20')) === 'false'),
    preferH264: (decodeURIComponent((new RegExp('[?|&]h264=' + '([^&;]+?)(&|#|;|$)').exec(location.search)||[,""])[1].replace(/\+/g, '%20')) === 'true'),
...
//    callStatsSecret: "", // Secret for callstats.io API
    ...
    //backToP2PDelay: 5
    recvonly: (decodeURIComponent((new RegExp('[?|&]recvonly=' + '([^&;]+?)(&|#|;|$)').exec(location.search)||[,""])[1].replace(/\+/g, '%20')) === 'true')
};
