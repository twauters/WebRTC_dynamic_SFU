    ...
    implements TransformEngine
{
	...
	try{
        	remb = makeREMB(senderSSRC);
	}
	catch(Exception e)
	{
		logger.error("exception makeREMB.", e);
	}	
    ...
    private RTCPREMBPacket makeREMB(long senderSSRC)
    {
    	...
    	Integer[] ssrcsint = (Integer[])(ssrcs.toArray(new Integer[ssrcs.size()]));
    	
    	Arrays.sort(ssrcsint);

    	for(int i=0; i < ssrcs.size(); i++){
    		dest[i] = ssrcsint[i] & 0xFFFFFFFFL;
    	}	
    	
    	long bitrate = -1;
    	
    	if(stream instanceof VideoMediaStreamImpl){
    		SenderBitrateController senderBitrateController = ((VideoMediaStreamImpl) stream).getSenderBitrateController();
    		bitrate = senderBitrateController.getLatestBitrate(dest[0]);
    		logger.debug("[SenderController] Getting latest bitrate from SenderBitrateController for ssrc " + dest[0] + " : " + bitrate);
    	}
    	if(bitrate == -1){
    		logger.debug("[SenderController] Getting latest bitrate from RemoteBitrateEstimator for : " + dest[0]);
    		bitrate = remoteBitrateEstimator.getLatestEstimate();
    	}
