...
const SIMULCAST_LAYERS = (decodeURIComponent((new RegExp('[?|&]layers=' + '([^&;]+?)(&|#|;|$)').exec(location.search)||[,""])[1].replace(/\+/g, '%20')) || 3)*1;
...
= function(mediaType) {
    ...
    if (decodeURIComponent((new RegExp('[?|&]sendonly=' + '([^&;]+?)(&|#|;|$)').exec(location.search)||[,""])[1].replace(/\+/g, '%20')) === 'true'){
	return 'sendonly';
    }
