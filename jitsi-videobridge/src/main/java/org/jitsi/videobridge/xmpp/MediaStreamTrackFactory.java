...
public class MediaStreamTrackFactory
{
    ...
    private static final org.jitsi.util.Logger logger = org.jitsi.util.Logger.getLogger(MediaStreamTrackFactory.class);
	...
	logd("Creating RTPEncodings: # spatial layers: " + spatialLen + ", # temporal layers: " + temporalLen + ", primary: " + primary.length);	
	...
	String msg = "rtpEncodings: \n";
	for(int i=0; i < rtpEncodings.length; i++){
		msg+=i+": " + rtpEncodings[i].toString() + ",frame_rate="+ rtpEncodings[i].getFrameRate() + ",height="+rtpEncodings[i].getHeight()+"\n";		
	}
	logd(msg);
