    ...
    implements TransformEngine
{
    ...
    private Endpoint forwarded;
    public void setForwardedEndpoint(Endpoint forwarded){
	if(this.forwarded != forwarded){
		this.forwarded = forwarded;
	}
    }

    public Endpoint getForwarded(){
	   return forwarded;
    }
	...
	if(forwarded != null && conferenceEndpoints.get(0) != dest.getEndpoint()){
		int index = conferenceEndpoints.indexOf(forwarded);		
		if(index != 0){
			Endpoint notForwarded = conferenceEndpoints.get(0);
			conferenceEndpoints.set(0,forwarded);
			conferenceEndpoints.set(index,notForwarded);
		}
	}
       ...
       if (bwe != null && bweBps == -1 && trustBwe)
        ...
        {
	    logger.debug("Forwarded Endpoints changed for VideoChannel " + dest.getID());
