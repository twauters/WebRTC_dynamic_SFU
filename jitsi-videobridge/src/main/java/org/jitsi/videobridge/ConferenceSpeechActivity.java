    ...
    implements PropertyChangeListener
{
	...
	MediaType m = MediaType.VIDEO;
		...
		//if(first){
			...
			//		logger.debug("Dominant speaker: 10 seconds past");				
			//		List<Endpoint> endpoints = getConference().getEndpoints();
			//		logger.debug("Dominant speaker: endpoints " + endpoints.size());
			//		Endpoint dominant = getDominantEndpoint();
			//		logger.debug("current dominant speaker id: " + dominant.getID() + " SSRC: " + dominant.getChannels(MediaType.VIDEO).get(0).getReceiveSSRCs()[0]);
			//
			//		int s=-1;
			//		for(Endpoint end:endpoints){
			//			//logger.debug("Dominant speaker changing...");
			//			VideoChannel v= (VideoChannel) end.getChannels(MediaType.VIDEO).get(0);
			//			if(end.getChannels(MediaType.VIDEO).get(0).getReceiveSSRCs().length > 0){
			//				logger.debug("Dominant speaker: receiver audio SSRCs for endpoint " + end.getID() + ": " + Arrays.toString(end.getChannels(MediaType.AUDIO).get(0).getReceiveSSRCs()));
			//				logger.debug("Dominant speaker: receiver video SSRCs for endpoint " + end.getID() + ": " + Arrays.toString(end.getChannels(MediaType.VIDEO).get(0).getReceiveSSRCs()));
			//				if(!end.equals(getDominantEndpoint()) && v.getLastN() == 0){
			//					s=end.getChannels(MediaType.VIDEO).get(0).getReceiveSSRCs()[0];
			//				}
			//			}
			//			else{
			//				logger.debug("Dominant speaker: no receiver SSRCs for endpoint " + end.getID());
			//			}
			//			
			//		}
			//		logger.debug("Dominant speaker changed to: " + s);
			//		activeSpeakerChanged(s & 0xFFFFFFFFL);
			//	} 
			//});
			//t.start();
			//first=false;
		//}
            ...
            Endpoint endpoint = conference.findEndpointByReceiveSSRC(ssrc, m);
            ...
            synchronized (syncRoot)
            {
                if (endpoint == null)
                {
                     ...
                     * HERE.
                     */
		    logger.debug("Dominant endpoint is null");
